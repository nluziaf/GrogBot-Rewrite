...
@bot.event
async def on_ready():
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("with Luziaf#0001 :)"))
...
slurs = ["nigger", "nigga", "faggot", "retard", "kys", "fatherless", "parentless"]
...
@tree.command(guild=discord.Object(id=966238887456428052),
...
class Picture(app_commands.Group):
    @app_commands.command(name="invert", description="Invert the avatar")
    async def invert(self, interaction: discord.Interaction, member: discord.Member = None):
        if member is None:
            member = interaction.user

        async with aiohttp.ClientSession() as invsession:
            async with invsession.get(f'https://some-random-api.ml/canvas/invert?avatar={member.display_avatar.replace(format="png", size=1024)}') as invimg:
                image = io.BytesIO(await invimg.read())

                await invsession.close()
                await interaction.response.send_message(file=discord.File(image, 'image.png'))

    @app_commands.command(name="greyscale", description="Greyscale the avatar")
    async def grayscale(self, interaction: discord.Interaction, member: discord.Member = None):
        if member is None:
            member = interaction.user

        async with aiohttp.ClientSession() as invsession:
            async with invsession.get(
                    f'https://some-random-api.ml/canvas/greyscale?avatar={member.display_avatar.replace(format="png", size=1024)}') as invimg:
                image = io.BytesIO(await invimg.read())
...
class Animal(app_commands.Group):
    @app_commands.command(name="cat", description="Pet Image")
    async def cat(self, interaction: discord.Interaction):
        api = urllib.request.urlopen('https://some-random-api.ml/animal/cat')
        data = json.load(api)
    ...
    @app_commands.command(name="dog", description="Pet Image")
    async def dog(self, interaction: discord.Interaction):
        api = urllib.request.urlopen('https://some-random-api.ml/animal/dog')
        data = json.load(api)
    ...
    @app_commands.command(name="fox", description="Pet Image")
    async def fox(self, interaction: discord.Interaction):
        api = urllib.request.urlopen('https://some-random-api.ml/animal/fox')
        data = json.load(api)

        image = data['image']
        fact = data['fact']

        embed = discord.Embed(title="Here's a fox",
                              description=f'Did you know? {fact}',
                              colour=0xb27b56)
        embed.set_image(url=image)
        await interaction.response.send_message(embed=embed)

    @app_commands.command(name="panda", description="Pet Image")
    async def panda(self, interaction: discord.Interaction):
        api = urllib.request.urlopen('https://some-random-api.ml/animal/panda')
        data = json.load(api)
    ...
    @app_commands.command(name="redpanda", description="Pet Image")
    async def redpanda(self, interaction: discord.Interaction):
        api = urllib.request.urlopen('https://some-random-api.ml/animal/red_panda')
        data = json.load(api)

        image = data['image']
        fact = data['fact']

        embed = discord.Embed(title="Here's a red panda",
                              description=f'Did you know? {fact}',
                              colour=0xb27b56)
        embed.set_image(url=image)
        await interaction.response.send_message(embed=embed)

    @app_commands.command(name="koala", description="Pet Image")
    async def koala(self, interaction: discord.Interaction):
        api = urllib.request.urlopen('https://some-random-api.ml/animal/koala')
        data = json.load(api)
...
@tree.command(guild=discord.Object(id=966238887456428052),
              description="Get MC Server data")
async def mcserv(interaction: discord.Interaction, server: str):
    ...
    if server_data.status_code == 404:
        ...
        embed = discord.Embed(colour=0xb27b56)
        embed.set_author(name=f"Data for {server}")
...
@tree.command(guild=discord.Object(id=966238887456428052),
              description="Get MC Player data")
...
@tree.command(guild=discord.Object(id=966238887456428052),
              description="Generate memes from random meme subreddits")
...
@tree.command(guild=discord.Object(id=966238887456428052),
              description="Suggest anything, we'll hear your voices")
...
async def warn(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason"):
    ...
    await interaction.response.send_message(f"Warned {member} for {reason}, now they have {new_count} warn(s)")

# Loading some classes here
tree.add_command(Info(), guild=discord.Object(id=966238887456428052))
tree.add_command(Picture(), guild=discord.Object(id=966238887456428052))
tree.add_command(Animal(), guild=discord.Object(id=966238887456428052))
tree.add_command(Math(), guild=discord.Object(id=966238887456428052))

# CitizenshipBadge
@tree.command(guild=discord.Object(id=966238887456428052),
              description="Apply for The Grog's Realm Minecraft Server")
...
async def sync_test_slash():
    ...
    await tree.sync(guild=discord.Object(id=966238887456428052))
